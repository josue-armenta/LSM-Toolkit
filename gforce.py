...
class GForceProfile:
    ...
    def handle_disconnect(self, client: BleakClient, *args):
        """
        Callback para desconexión BLE.
        En Windows, BleakWinRT pasa (client, *args) al callback.
        """
    ...
    async def calibrate_quaternion(self, duration: float = 2.0):
        """
        Calibra la orientación tomando 'duration' segundos de quaterniones en reposo
        y computando un quaternion de referencia (offset) para alinear futuras lecturas.
        """
        buf = []

        def _on_quat_notify(_char, data: bytearray):
            # data[0] es el tipo, data[1:17] son 4 floats little‐endian
            qw, qx, qy, qz = struct.unpack("<4f", data[1:17])
            buf.append((qw, qx, qy, qz))

        # Suscribirse temporalmente a quaterniones
        await self.device.start_notify(DATA_NOTIFY_CHAR_UUID, _on_quat_notify)
        await asyncio.sleep(duration)
        await self.device.stop_notify(DATA_NOTIFY_CHAR_UUID)

        # Calcular quaternion medio y normalizar
        mean_q = np.mean(buf, axis=0)
        norm = np.linalg.norm(mean_q)
        if norm == 0:
            raise RuntimeError("Calibración de quaternion fallida: datos inválidos")
        self.quat_offset = np.array(mean_q) / norm
